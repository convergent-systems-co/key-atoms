...
Walks atoms/, keychains/, rules/; validates each against its schema; assembles
...
COMPOSITIONS_DIR = REPO / "keychains"
RULES_DIR = REPO / "rules"  # reserved; key-atoms does not declare ruleTypes today
EXPORT_PATH = REPO / "exports" / "catalog.json"
CATALOG_NAME = "key-atoms"
...
def main() -> int:
    atoms = collect(ATOMS_DIR, load_validator("atom-v1.1.json"), "atom")
    # composition and rule schemas for v1.1.0 are not yet authored; collected
    # without schema validation if the directory contains JSON files. The full
    # v1.1.0 composition/rule schemas are deferred to schema-atoms and a
    # follow-up commit by the principal.
