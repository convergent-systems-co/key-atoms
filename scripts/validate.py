...
SCHEMAS = {
    "atom":        REPO / "schemas" / "atom-v1.1.json",
    "composition": REPO / "schemas" / "composition-v1.1.json",  # not yet authored
    "rule":        REPO / "schemas" / "rule-v1.1.json",          # reserved; not yet authored
}
    ...
    for path in sorted(ATOMS_DIR.rglob("*.json")):
        ...
        if not local_errors:
            ...
            if data.get("type") != parent:
                ...
                key = f"key-atoms://atoms/{parent}/{data['id']}"
                atom_index[key] = data
                print(f"✓ {rel}")
        errors += local_errors
